...
const ACCEPTABLE_BACKGROUND_FILE_TYPES = ['image/jpeg', 'image/png', 'image/webp']
...
export default function ImagesProvider({children}: ImagesProviderProps) {
	...
	async function uploadRoomBackground(file: File) {
		if (!ACCEPTABLE_BACKGROUND_FILE_TYPES.includes(file.type)) throw new Error('Unsupported file type')
		const storage = getStorage(app)
