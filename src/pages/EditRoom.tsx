...
function EditRoomForm({room}: EditRoomFormProps) {
	...
	const [backgroundPath, setBackgroundPath] = useState<string | undefined>(room.backgroundPath)
		...
		<div className={styles.editRoom}>
			<div className={styles.editRoomContainer}>
				...
				<form onSubmit={handleSubmit}>
					<BackgroundField onChange={(path: string) => setBackgroundPath(path)} value={backgroundPath} />
					<TextField
						name='description'
					...
					<Button onClick={() => navigate(`/rooms/${room.name}`)}>Cancel</Button>
				</form>
			</div>
		</div>
	)
