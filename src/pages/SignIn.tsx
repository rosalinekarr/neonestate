...
function PhoneNumberForm({ onSubmit }: PhoneNumberFormProps) {
  ...
  function handleChange(newPhoneNumber: string) {
    setIsValid(
      (prevIsValid) => prevIsValid || isPhoneNumberValid(newPhoneNumber),
    );
  ...
  return (
    <form onSubmit={handleSubmit}>
      ...
      <IconButton
        ...
        id="sign-in"
        className={
          isValid ? styles.button : [styles.button, styles.error].join(" ")
        }
      />
    </form>
  );
...
function ConfirmationCodeForm({ onSubmit }: ConfirmationCodeFormProps) {
  ...
  function handleChange(newCode: string) {
    setIsValid((prevIsValid) => prevIsValid || newCode.length === 6);
...
export default function SignIn() {
  ...
  async function sendConfirmationCode(phoneNumber: string): Promise<void> {
    setConfirmation(
      await signInWithPhoneNumber(
        auth,
        phoneNumber,
        new RecaptchaVerifier(auth, "sign-in", {
  ...
  return (
    <div className={styles.signIn}>
      <h2>Sign in</h2>
      {confirmation !== null ? (
        <ConfirmationCodeForm onSubmit={verifyConfirmationCode} />
      ) : (
        <PhoneNumberForm onSubmit={sendConfirmationCode} />
      )}
    </div>
  );
}
